...
export const Header: React.FC = () => {
  ...
  useEffect(() => {
    DataService.getUnreadNotifications(currentEmployeeId).then(n => setUnreadCount(n.length));
    const interval = setInterval(() => DataService.getUnreadNotifications(currentEmployeeId).then(n => setUnreadCount(n.length)), 10000);
    return () => clearInterval(interval);
  }, [currentEmployeeId]);

  const roles: EmployeeRole[] = ['ADMIN', 'HOD', 'MANAGER', 'ENGINEER', 'TECHNICIAN'];
